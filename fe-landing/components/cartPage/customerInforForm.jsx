...
const CustomerInforForm = ({ email = '', customerName = '', phoneNumber = '', address = '', handlePlaceOrder }) => {
    ...
    const onSubmit = (data) => {
        handlePlaceOrder({ ...data, paymentMethod });
    };
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="payment">
                ...
                <div>
                    <label
                        htmlFor="cod"
                        className={`payment-item w-100 border-radius d-flex align-items-center justify-content-start ${paymentMethod === 'COD' ? 'active' : ''}`}
                        onClick={() => setPaymentMethod('COD')}
                        style={{ cursor: 'pointer', marginBottom: '1rem' }}
                    >
                        <div className="payment-item-radio">
                            <Radio checked={paymentMethod === 'COD'}></Radio>
                        </div>
            ...
            <div className={'btn-container' + (isSubmitting ? ' btn-loading' : '')}>
                <Button htmlType='submit' loading={isSubmitting}>
                    {!isSubmitting && (paymentMethod === 'VNPAY' ? 'Thanh Toán VNPay' : 'Đặt Hàng')}
                </Button>
            </div>
        </form>
    )
